...
<script>
    document.addEventListener('DOMContentLoaded', function () {
        ...
        const routeMapping = {
            'Informe Consolidado': {
                'PDF': '{{ route("report.pdf", ["startDate" => "__START__", "endDate" => "__END__"]) }}',
                'Excel': '{{ route("report.excel", ["startDate" => "__START__", "endDate" => "__END__"]) }}',
                'Ver en la Web': '{{ route("consolidado.ver", ["startDate" => "__START__", "endDate" => "__END__"]) }}',
            },
            'Entregados': {
                'PDF': '{{ route("report.entregadas.pdf", ["startDate" => "__START__", "endDate" => "__END__"]) }}',
                'Excel': '{{ route("report.entregadas.excel", ["startDate" => "__START__", "endDate" => "__END__"]) }}',
                'Ver en la Web': '{{ route("ver.entregadas.pdf", ["startDate" => "__START__", "endDate" => "__END__"]) }}',
            },
            'Solicitados': {
                'PDF': '{{ route("report.solicitadas.pdf", ["startDate" => "__START__", "endDate" => "__END__"]) }}',
                'Excel': '{{ route("report.solicitadas.excel", ["startDate" => "__START__", "endDate" => "__END__"]) }}',
                'Ver en la Web': '{{ route("ver.solicitadas.pdf", ["startDate" => "__START__", "endDate" => "__END__"]) }}',
            },
        };
        ...
        function showStep(index) {
            ...
            currentStep = index;
            backStepBtn.classList.toggle('hidden', currentStep === 0);
        }
        ...
        document.querySelectorAll('.option-btn').forEach(button => {
            button.addEventListener('click', (e) => {
                selectedOption = e.target.innerText.trim();
                showStep(1);
            });
        });
        ...
        document.querySelectorAll('.format-btn').forEach(button => {
            button.addEventListener('click', (e) => {
                selectedFormat = e.target.innerText.trim();

                // Obtener ruta basada en la opción y formato
                const baseRoute = routeMapping[selectedOption][selectedFormat];
                if (!baseRoute) {
                    alert('Opción o formato no válido.');
                    return;
                }

                // Reemplazar placeholders con fechas reales
                const finalRoute = baseRoute.replace('__START__', startDate).replace('__END__', endDate);
                window.location.href = finalRoute; // Redirige a la ruta generada
            });
        });
    });
</script>
